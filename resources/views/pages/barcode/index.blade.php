...
@section('content')
    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title">Data Barcode Asset</h5>
            <div class="header-elements">
                <div class="list-icons">
        ...
        <div class="card-body">
        	<div class="row" style="overflow-x: scroll;">
                <div class="col-md-12">
                    <button id="download-btn" class="btn btn-success btn-sm"><i class="fa fa-check-circle"></i> Download
                    </button>
                </div>
                <br>       
            	<br>
...
<script type="text/javascript">
    ...
    $("#download-btn").click(function(){
        // console.log('test');
        var verf = [];
        $.each($("input[name='download_barcode']:checked"), function(){
            verf.push($(this).val());
        });
        let strid = verf.join();
        // console.log(strid);
        window.location.href = "{{ url('barcodeaset/download') }}/" + strid;
    });
    ...
    $("#filterDep").on("submit", function(event){
        ...
        if (formValues) {
            $('#tabel').DataTable().ajax.url("/barcodeaset/data?" + formValues).load();
        }
    });
    // end:submit filter
</script>
