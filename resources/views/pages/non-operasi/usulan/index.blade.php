...
@section('title', 'Usulan Pekerjaan - Asset Management')

@section('pagetitle', 'Usulan Pekerjaan')

@section('content') 
        <!-- BEGIN EXAMPLE TABLE PORTLET-->
        <div class="card {{($data=='')?'':'hidden'}}" id="instalasi-data">
            <div class="card-header header-elements-inline">
                <h5 class="card-title">List Usulan Pekerjaan</h5>
            </div>
            ...
            <div class="card-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-10">
                        {!! Form::select('rencanaAnggaran', $rencanaAnggaran, null, ['class'=>'form-control select2', 'id'=>'anggaran']) !!}
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <label class="col-form-label col-lg-2">SPV Pelaksana</label>
                    <div class="col-lg-10">
                ...
                <div class="form-group row">
                    <div class="col-lg-12">
                        <button class="btn btn-primary legitRipple" id="cari">Cari</button> 
                    </div>
                </div>
                <br>
				
                @if (!in_array(namaRole(), config('custom.roleExceptionNoAdmin')))
                    <a href="{{ route('non-operasi::usulan-entri') }}" id="tambah-btn" class="btn btn-success legitRipple"><i class="fa fa-plus"></i> Tambah Baru</a>
                @endif
                ...
                <table class="table datatable-basic" id="tabel">
                    <thead>
                        <tr>
                            <th>ID</th>     
                            <th>Rencana<br>Penggunaan<br>Anggaran</th>     
                            <th>Judul Usulan</th>    
                            <th>Nama Aset</th>    
                            <th>SPV Pelaksana</th>    
                            <th>Instalasi</th>
...
//    alert('aa');
    ...
    $('#tabel').DataTable({
        "processing": true,
        "serverSide": true,
        "ajax": "{{ route('non-operasi::usulan-data') }}",
        "columns": [    
            {data: 'id', name: 'id', defaultContent: '-'},        
            {data: 'penggunaan_anggaran', name: 'penggunaan_anggaran', defaultContent: '-'},
            {data: 'nama', name: 'nama', defaultContent: '-'},
            {data: 'aset.nama_aset', name: 'aset', defaultContent: '-', orderable: false, searchable: false},
            {data: 'jabatan.namajabatan', name: 'jabatan', defaultContent: '-', orderable: false, searchable: false},
    ...
    /*$('#year').change(function() {
        if ($(this).val() != '') {
            console.log('test');
            $('#tabel').DataTable().ajax.url("{{ url('NonOperasi/usulan-data') }}?period=" + $(this).val() + '&status=' + $("#status").val()).load();
        }
    });

    $('#status').change(function() {
        if ($(this).val() != '') {
            $('#tabel').DataTable().ajax.url("{{ url('NonOperasi/usulan-data') }}?period=" + $("#year").val() + '&status=' + $(this).val()).load();
        }
    });*/
    ...
    $('#cari').click(function() {
        ...
        $('#tabel').DataTable().ajax.url("{{ url('NonOperasi/usulan-data') }}?period=" + tahun + '&status=' + status + '&anggaran=' + anggaran + '&instalasi=' + instalasi + '&spv=' + spv).load();
    });
});
</script>
@endsection